document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', (e) => {
            ...
            let message = '';
            if (type === 'free-session') {
                message = `${name}様、合同無料計測会への参加申込を承りました。公式LINEでの調整またはメールにて詳細をお送りします。`;
            } else if (type === 'team-trial') {
                message = `${name}様、チームサブスクの体験デモ測定に関するお問い合わせを承りました。担当者より追ってご連絡差し上げます。`;
            } else {
                message = `${name}様、お問い合わせを承りました。返信をお待ちください。`;
            }
